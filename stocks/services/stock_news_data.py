...
class StockNewsDataProvider:
    ...
    def _llm_classify_news(self, news_list: List[Dict]) -> List[Dict]:
        """LLM으로 투자 관련 뉴스 분류 (2단계). OpenAI primary → Gemini fallback."""
        if not news_list:
            return news_list
        ...
        text = generate(prompt, temperature=0.0, max_output_tokens=256)
        if not text:
            return news_list
        try:
    ...
    def _translate_to_korean(self, company_name: str, english_summary: str) -> str:
        ...
        prompt = (
            f"다음은 '{company_name}' 종목에 대한 시장 반응 요약입니다. "
            "이 내용을 투자자가 이해하기 쉽게 한국어 2-3문장으로 번역해주세요.\n\n"
            f"{english_summary}\n\n"
            "조건:\n- 자연스러운 한국어로 번역\n- 투자 관련 핵심 내용 유지"
        )
        return generate(prompt, temperature=0.2, max_output_tokens=512) or ""
